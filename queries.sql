...
-- DATA FILTERING
    ...
    -- SEARCHING WITH "LIKE" AND "%" CHARACTER (JOKER) //! -- USE ONLY ON EMERGENCY, IT SLOWS DOWN YOUR PROCESSING -- !
...
-- COUNTING AND GROUPING -- WITHOUT LOOSING PERFORMANCE
    
    SELECT COUNT(*) AS "QUANTIDADE DE REGISTROS" FROM CLIENTE;
    SELECT SEXO, COUNT(*) AS "QUANTIDADE DE REGISTROS" FROM CLIENTE 
        GROUP BY SEXO;
    SELECT ESTADO, COUNT(*) AS "QUANTIDADE" FROM CLIENTE 
        GROUP BY ESTADO;

    -- COUNTING AND GROUPING W/ CONDITIONS

        -- AND 

            SELECT NOME, SEXO, ESTADO, COUNT(*) AS "QUANTIDADE" FROM CLIENTE 
        ...
        -- OR
            ...
            SELECT NOME, SEXO, ESTADO, COUNT(*) AS "QUANTIDADE" FROM CLIENTE 
                WHERE SEXO = 'F' OR ESTADO = 'SP'
                GROUP BY NOME;
-- DELETING DATA

    /* INSERT INTO CLIENTE VALUES ('Jonas Augusto','M','Jonas@gmail.com',734569112,'72345589','PR'); */

    DELETE FROM CLIENTE WHERE NOME LIKE '%AUGUSTO';